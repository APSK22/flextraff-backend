...
class TestAPIPerformance:
    ...
    @pytest.mark.asyncio
    async def test_single_calculation_performance(self, aio_session: aiohttp.ClientSession):
...
class TestAPILoadTesting:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_calculations_load(self, aio_session: aiohttp.ClientSession):
    ...
    async def test_sustained_load(self, aio_session: aiohttp.ClientSession):
        ...
        workers = 5
        start_time = time.time()
        worker_results = await asyncio.gather(*[sustained_request_worker() for _ in range(workers)])
        total_time = time.time() - start_time
        ...
        print(f"Sustained load test results ({workers} workers, {total_requests} total requests):")
        print(f"  Success rate: {success_rate:.1f}%")
        print(f"  Average response time: {avg_response_time:.2f}ms")
        print(f"  Total time: {total_time:.2f}s")
        print(f"  Throughput: {throughput:.1f} req/s")
        ...
        assert success_rate >= 98, f"Sustained success rate too low: {success_rate:.1f}%"
        assert avg_response_time < 150, f"Sustained response time too high: {avg_response_time:.2f}ms"
    ...
    async def test_mixed_workload(self, aio_session: aiohttp.ClientSession):
        ...
        async def mixed_request_worker(worker_id: int):
            ...
            for i in range(15):  # 15 requests per worker
                request_type = random.choice([
                    "calculation", "vehicle_detection", "status", 
                    "live_timing", "history", "health"
                ])
        ...
        workers = 8
        start_time = time.time()
        worker_results = await asyncio.gather(*[mixed_request_worker(i) for i in range(workers)])
        total_time = time.time() - start_time
        ...
        overall_success_rate = len([r for r in all_results if r["success"]]) / len(all_results) * 100
        
        print(f"Mixed workload test results ({workers} workers, {len(all_results)} total requests):")
        ...
        assert overall_success_rate >= 95, f"Overall success rate too low: {overall_success_rate:.1f}%"
...
class TestAPIStabilityTesting:
    ...
    async def test_edge_case_inputs(self, aio_session: aiohttp.ClientSession):
            ...
            try:
                async with aio_session.post(
                    ...
                    if response.status == 200:
                        ...
                        success_count += 1
                        print(f"Edge case {i+1}: {lane_counts} -> {data['green_times']} ({data['cycle_time']}s)")
        ...
        assert success_rate >= 80, f"Edge case success rate too low: {success_rate:.1f}%"