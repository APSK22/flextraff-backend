...
def assert_response_schema(response_data: dict, expected_schema: dict):
    ...
    for key, expected_type in expected_schema.items():
        ...
        assert isinstance(response_data[key], expected_type), f"Wrong type for {key}: expected {expected_type}, got {type(response_data[key])}"
...
def assert_valid_green_times(green_times: list):
    ...
    assert all(isinstance(t, int) and t >= 15 for t in green_times), "All green times should be integers >= 15"
...
def assert_valid_cycle_time(cycle_time: int):
    ...
    assert 60 <= cycle_time <= 180, f"Cycle time should be between 60-180s, got {cycle_time}"