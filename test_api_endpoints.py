...
async def test_api_endpoints():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(f"{BASE_URL}/health") as response:
            if response.status == 200:
                data = await response.json()
                print(f"   ✅ Health: {data['status']}, DB: {data['database_connected']}")
            else:
        ...
        async with session.get(f"{BASE_URL}/junction/{junction_id}/status") as response:
            if response.status == 200:
                ...
                if data['latest_cycle']:
                    print(f"   Latest cycle: {data['latest_cycle']['total_cycle_time']}s")
            else:
                print(f"   ❌ Junction status failed: {response.status}")
        
        # Test 7: Live timing
        print("7️⃣ Testing live timing...")
        async with session.get(f"{BASE_URL}/junction/{junction_id}/live-timing") as response:
            if response.status == 200:
                data = await response.json()
        ...
        async with session.get(f"{BASE_URL}/junction/{junction_id}/history?limit=5") as response:
            if response.status == 200:
                data = await response.json()
        ...
        async with session.get(f"{BASE_URL}/analytics/daily-summary") as response:
            if response.status == 200:
                ...
                for summary in data['junction_summaries']:
                    print(f"   - {summary['junction_name']}: {summary['total_vehicles']} vehicles")
            else:
        ...
        for scenario_name, lane_counts in test_scenarios:
            ...
            async with session.post(
                ...
                if response.status == 200:
                    data = await response.json()
                    print(f"   ✅ {scenario_name}: {data['green_times']} ({data['cycle_time']}s)")
                else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        import subprocess
        subprocess.check_call(["/Users/ajaypsk2722/flextraff-backend/.venv/bin/python", "-m", "pip", "install", "aiohttp"])
        import aiohttp
